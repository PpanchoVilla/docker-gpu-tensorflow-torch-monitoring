...
# JupyterLab and data science stack (PŘIDÁNO: torch, gym, scikit-learn, matplotlib, seaborn, tensorboard)
RUN pip install --no-cache-dir --timeout 300 --retries 5 \
    jupyterlab notebook ipywidgets \
    numpy scipy pandas matplotlib seaborn scikit-learn \
    torch gymnasium tensorboard tensorboard-plugin-profile --break-system-packages

# nvitop and additional dependencies
RUN pip install --no-cache-dir --timeout 300 --retries 5 \
    nvitop fastapi uvicorn --break-system-packages

# PŘIDÁN NOVÝ KROK: Finanční a volitelné ML knihovny (TA-Lib, wandb)
RUN pip install --no-cache-dir --timeout 300 --retries 5 \
    TA-Lib wandb --break-system-packages
